...
# Canary sing-box uninstaller entrypoint. It never removes host Xray or the
# Docker engine and only accepts an installation record written by install-v2.
...
entrypoint_cleanup() {
	[ -z "$ONE_NODE_V2_ENTRYPOINT_TEMP_DIR" ] ||
		rm -rf -- "$ONE_NODE_V2_ENTRYPOINT_TEMP_DIR"
}
...
entrypoint_load_modules() {
	...
	if [ -z "$source_dir" ]; then
		ONE_NODE_V2_ENTRYPOINT_TEMP_DIR=$(mktemp -d "/tmp/one-node-v2-uninstaller.XXXXXX")
