...
validate_install_target() {
	...
	[ ! -e "$INSTALL_RECORD" ] ||
		die "a v2 installation already exists; upgrades belong to the upgrade workflow"
	[ ! -e "/opt/one-node-node/.installation" ] ||
		die "a legacy installation exists; uninstall it before installing v2"
