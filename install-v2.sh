...
ONE_NODE_V2_INSTALL_MODULES="common.sh config.sh host.sh files.sh native.sh docker.sh readiness.sh main.sh"
...
entrypoint_local_source_dir() {
	...
	*) return 1 ;;
	...
	entrypoint_dir=$(CDPATH='' cd -- "$(dirname "$0")" 2>/dev/null && pwd) ||
		return 1
	source_dir="${entrypoint_dir}/scripts/node-v2/install"
...
entrypoint_module_base_url() {
	...
	base_url=${base_url%/}
	case "$base_url" in
	*/scripts/node-v2) base_url="${base_url}/install" ;;
	*/scripts/node-v2/install) ;;
